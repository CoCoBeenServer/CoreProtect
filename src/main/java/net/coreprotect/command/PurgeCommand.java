...
public class PurgeCommand extends Consumer {
    ...
    protected static void runCommand(final CommandSender player, boolean permission, String[] args) {
        ...
        if (resultc <= 1) {
            Chat.sendMessage(player, Color.DARK_AQUA + "容器操作紀錄 " + Color.WHITE + "- " + Phrase.build(Phrase.MISSING_PARAMETERS, "/co purge t:<time>"));
            return;
        }
        if (endTime <= 0) {
            Chat.sendMessage(player, Color.DARK_AQUA + "容器操作紀錄 " + Color.WHITE + "- " + Phrase.build(Phrase.MISSING_PARAMETERS, "/co purge t:<time>"));
        ...
        for (int action : argAction) {
            if (!supportedActions.contains(action)) {
                Chat.sendMessage(player, Color.DARK_AQUA + "容器操作紀錄 " + Color.WHITE + "- " + Phrase.build(Phrase.ACTION_NOT_SUPPORTED));
        ...
        if (player instanceof Player && endTime < 2592000) {
            Chat.sendMessage(player, Color.DARK_AQUA + "容器操作紀錄 " + Color.WHITE + "- " + Phrase.build(Phrase.PURGE_MINIMUM_TIME, "30", Selector.FIRST)); // 30 days
            return;
        }
        else if (endTime < 86400) {
            Chat.sendMessage(player, Color.DARK_AQUA + "容器操作紀錄 " + Color.WHITE + "- " + Phrase.build(Phrase.PURGE_MINIMUM_TIME, "24", Selector.SECOND)); // 24 hours
            return;
        }
